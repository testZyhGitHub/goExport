// 命令行标志(使用标记的命令行处理方法, 命令行标志是命令行程序指定选项的常用方式。	例如, 在wc -l中, 这个-l就是一个命令行标志。)
package main

// Go提供了一个flag包, 支持基本的命令行标志解析, 我们将用这个包来实现我们的命令行程序示例。
import "flag"
import "fmt"
...
func main1() {
	...
	// 这里我们声明一个默认值为"foo"的字符串标志word并带有一个简短的描述。
	// 这里的flag.String函数返回一个字符串指针(不是一个字符串值), 在下面我们会看到是如何使用这个指针的。
    wordPtr := flag.String("word", "foo", "a string")
	
	// 使用和"声明word标志相同的方法"来声明numb和fork标志。
	numbPtr := flag.Int("numb", 42, "an int")
	boolPtr := flag.Bool("fork", false, "a bool")
	
	// 用"程序中已有的参数"来声明一个标志也是可以的, 注意在标志声明函数中需要使用该参数的指针。
	var svar string
